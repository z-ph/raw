...
def load_data():
    # 加载附件1数据
    enterprise_info = pd.read_csv('附件1：123家有信贷记录企业的相关数据.xlsx_0.csv')
    sales_invoices = pd.read_csv('附件1：123家有信贷记录企业的相关数据.xlsx_1.csv')
    purchase_invoices = pd.read_csv('附件1：123家有信贷记录企业的相关数据.xlsx_2.csv')
    
    # 加载附件2数据（无信贷记录企业）
    try:
        non_credit_enterprises = pd.read_csv('附件2：302家无信贷记录企业的相关数据.xlsx_0.csv')
        # 尝试加载附件2的发票数据（处理可能的读取错误）
        try:
            non_credit_sales = pd.read_csv('附件2：302家无信贷记录企业的相关数据.xlsx_1.csv')
        except:
            non_credit_sales = pd.DataFrame(columns=['企业代号', '金额', '发票状态'])
        
        try:
            non_credit_purchases = pd.read_csv('附件2：302家无信贷记录企业的相关数据.xlsx_2.csv')
        except:
    ...
    # 加载附件3数据
    interest_rates = pd.read_csv('附件3：银行贷款年利率与客户流失率关系的统计数据.xlsx_0.csv')
