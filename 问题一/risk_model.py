...
# 数据预处理
# 1. 信誉评级映射为数值 (A:0.1, B:0.3, C:0.6, D:1)
rating_mapping = {'A': 0.1, 'B': 0.3, 'C': 0.6, 'D': 1}
temp = df['信誉评级']
df['信誉评级'] = df['信誉评级'].map(rating_mapping)
# 2. 选择特征并归一化
features = ['信誉评级', '发票作废率', '负数发票占比', '净现金流', '进销项比', '金额标准差', '月交易额波动率', '最大断月数', '合作企业数量', '活跃天数', '发票总数量', '金额总和', '金额均值', '月交易额平均', '最大月增长', '月增长率中位数', '近3月增长率', '活跃月数','是否违约']
...
def calculate_ahp_weights():
    # 选取的10个关键风险字段
    selected_features = [
        '信誉评级', '净现金流','发票总数','金额总数','合作企业数量','近3月增长率',
        '是否违约','进销项比','发票作废率','负数发票占比'
    ]
    
    # 对应的AHP判断矩阵（10x10）
    ahp_matrix = [
        [1, 2, 5, 4, 5, 3, 1/3, 2, 2, 2],          # 信誉评级
        [1/2, 1, 5, 3, 4, 3, 1/3, 1, 3, 3],         # 净现金流
        [1/5, 1/5, 1, 1/2, 1/3, 1/3, 1/7, 1/4, 1/4, 1/4],  # 发票总数
        [1/4, 1/3, 2, 1, 1/2, 1/2, 1/6, 1/3, 1/3, 1/3],    # 金额总数
        [1/5, 1/4, 3, 2, 1, 1/2, 1/7, 1/3, 1/3, 1/3],      # 合作企业数量
        [1/3, 1/3, 3, 2, 2, 1, 1/5, 1/2, 1/2, 1/2],        # 近3月增长率
        [3, 3, 7, 6, 7, 5, 1, 4, 4, 4],             # 是否违约（最重要指标）
        [1/2, 1, 4, 3, 3, 2, 1/4, 1, 2, 2],         # 进销项比
        [1/2, 1/3, 4, 3, 3, 2, 1/4, 1/2, 1, 1],     # 发票作废率
        [1/2, 1/3, 4, 3, 3, 2, 1/4, 1/2, 1, 1]      # 负数发票占比
    ]
    
    # 使用判断矩阵计算权重
    judgment_matrix = np.array(ahp_matrix)
...
# 隶属度函数（梯形）
def 隶属度(x, feature):
    if feature not in[ '发票作废率', '负数发票占比', '进销项比',  '月交易额波动率', '最大断月数','是否违约']:
          # 越高风险越小
...
max_score = df['评分分数'].max()
mix_score = df['评分分数'].min()
df['贷款金额(单位:万元)'] = (10 + ( df['评分分数'] - mix_score ) * (100 - 10) / (max_score - mix_score)).round(2)
# 保存结果
df[['企业代号', '企业名称', '风险系数','信誉评级', '评分分数','贷款金额(单位:万元)']].to_csv(output_path, index=False,encoding='utf-8-sig')
